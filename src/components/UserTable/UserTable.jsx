...
import {
  ...
  Paper, IconButton, Typography, CircularProgress, Box, Tooltip, Dialog,
  DialogTitle, DialogContent, DialogActions, Button, TextField, MenuItem
} from "@mui/material";
import { Edit, Delete } from "@mui/icons-material";
import { motion } from "framer-motion";
import { getAllUsers, updateUserDetails, deleteUser } from "../../utils/services/api";
...
const UserTable = () => {
  ...
  const handleEdit = (user) => {
    ...
    setFormData({ role: user.role, department: user.department?._id || "" });
    setOpenEdit(true);
  };
          ...
          <TableBody>
            {users.map((user) => (
              <TableRow key={user._id}>
                ...
                <TableCell>
                  {user.role === 0 ? "Citizen" : user.role === 1 ? "Gov Agent" : "Admin"}
                </TableCell>
